...
function Meteogram(json, container) {
    ...
    this.symbols = [];
    this.humidities = [];
    this.humiditiesError = []; // Only for some data sets
    this.winds = [];
    this.temperatures = [];
    this.pressures = [];
    ...
    return {
        ...
        series: [{
            ...
            tooltip: {
                ...
                pointFormat: '<span style="color:{point.color}">\u25CF</span>' +
                    ' ' +
                    '{series.name}: <b>{point.minvalue} in - ' +
                    '{point.maxvalue} in</b><br/>'
            },
            grouping: false,
            dataLabels: {
...
Meteogram.prototype.parseYrData = function () {
    ...
    this.json.properties.timeseries.forEach((node, i) => {
        ...
        this.humidities.push({
            x,
            y: nextHours.details.relative_humidity
        });
        // console.log(`nextHours=${JSON.stringify(nextHours)}`)
        // console.log(`humidity=${nextHours.details.relative_humidity}`)
...
function displayChartHourly(weatherData) {
    console.log(`DBG: displayChartHourly`);

    new Meteogram(weatherData, 'weather-chart-hourly');
    return;
    ...
    Highcharts.ajax({
        ...
        success: json => {
            ...
            window.meteogram = new Meteogram(json, 'weather-chart-hourly');
        },
